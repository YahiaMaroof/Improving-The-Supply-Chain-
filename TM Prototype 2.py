...
class SupplyChainAppCase2:
    def __init__(self, root):
        self.root = root
        self.root.title("نظام مقارنة طرق سلاسل الإمداد - الحالة الثانية")
    ...
    def create_widgets(self):
        ...
        tk.Label(header_frame, text="نظام مقارنة طرق سلاسل الإمداد - الحالة الثانية", 
                font=('Arial', 18, 'bold'), bg='#f0f0f0').pack()
        
        tk.Label(header_frame, text="مشروع التخرج - تحسين سلاسل الإمداد لقطع الغيار (مقارنة الرحلات الفردية)", 
    ...
    def compare_methods(self):
        ...
        # Method 2: Each supplier makes a round trip (NEW - Case 2 logic)
        individual_distance, shipping_details = self.calculate_individual_round_trip_distance(selected_suppliers)
        
        self.results_text.insert(tk.END, "📦 الطريقة الثانية: كل مورد يرسل ويعود (رحلة ذهاب وإياب)\n")
        self.results_text.insert(tk.END, "-" * 50 + "\n")
        
        for detail in shipping_details:
            self.results_text.insert(tk.END, f"{detail['supplier_name']}: {detail['distance_to_supplier']:.1f} + {detail['distance_back']:.1f} = {detail['round_trip_distance']:.1f} كم\n")
        ...
        self.results_text.insert(tk.END, f"مسافة الرحلات الفردية (ذهاب وإياب): {individual_distance:.1f} كم\n")
        ...
        if savings > 0:
            ...
            self.results_text.insert(tk.END, "   السبب: المسار الأمثل أقصر من الرحلات الفردية\n")
        elif savings < 0:
            self.results_text.insert(tk.END, f"✅ استخدم الرحلات الفردية - توفير {abs(savings):.1f} كم ({abs(savings_percentage):.1f}%)\n")
            self.results_text.insert(tk.END, "   السبب: الرحلات الفردية أقصر من المسار الأمثل\n")
        else:
    ...
    def calculate_individual_round_trip_distance(self, supplier_letters, warehouse='A'):
        """Calculate total distance if each supplier makes a round trip (warehouse -> supplier -> warehouse)"""
        total_distance = 0
        shipping_details = []
        
        for supplier in supplier_letters:
            if supplier != warehouse:  # Don't count warehouse to itself
                # Round trip: warehouse -> supplier -> warehouse
                distance_to_supplier = self.distance_matrix[warehouse][supplier]
                distance_back_to_warehouse = self.distance_matrix[supplier][warehouse]
                round_trip_distance = distance_to_supplier + distance_back_to_warehouse
                
                total_distance += round_trip_distance
                shipping_details.append({
                    'supplier': supplier,
                    'supplier_name': SUPPLIER_MAP[supplier],
                    'distance_to_supplier': distance_to_supplier,
                    'distance_back': distance_back_to_warehouse,
